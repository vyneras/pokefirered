...
struct PokedexScreenData
{
    ...
    u16 dexsanityOrderMenuItemsAbove;
    u16 dexsanityOrderMenuCursorPos;
    ...
    struct EncounterAreaInfo * encounterAreaInfo;
    struct EvolutionInfo * evolutionInfo;
};
...
static void DexScreen_PrintMonHMCompatibility(u8 windowId, u8 fontId, u16 species, u8 x, u8 y);
static void DexScreen_PrintMonHMCompatibilityNumbers(u8 windowId, u8 fontId, u16 species, u8 x, u8 y);
...
u8 DexScreen_DrawMonInfoPage(void);
...
u8 DexScreen_DestroyInfoScreenResources(void);
...
void DexScreen_DisplayNextAreaPage(void);
void DexScDexScreen_DisplayPreviousAreaPage(void);
void DexScreen_DisplayAreaPage(void);
void DexScreen_DisplayNextEvolutionPage(void);
void DexScDexScreen_DisplayPreviousEvolutionPage(void);
void DexScreen_DisplayEvolutionPage(void);
...
static const struct ListMenuItem sListMenuItems_KantoDexsanityDexModeSelect[] = {
    {gText_PokemonList,                  LIST_HEADER},
    {gText_NumericalMode,                DEX_MODE(NUMERICAL_KANTO)},
    {gText_NumericalModeDexsanity,       DEX_MODE(NUMERICAL_DEXSANITY)},
    {gText_PokemonHabitats,              LIST_HEADER},
    {gText_DexCategory_GrasslandPkmn,    DEX_CATEGORY_GRASSLAND},
    {gText_DexCategory_ForestPkmn,       DEX_CATEGORY_FOREST},
    {gText_DexCategory_WatersEdgePkmn,   DEX_CATEGORY_WATERS_EDGE},
...
static const struct ListMenuTemplate sListMenuTemplate_KantoDexsanityDexModeSelect = {
    .items = sListMenuItems_KantoDexsanityDexModeSelect,
    .moveCursorFunc = MoveCursorFunc_DexModeSelect,
    .itemPrintFunc = ItemPrintFunc_DexModeSelect,
    .totalItems = NELEMS(sListMenuItems_KantoDexsanityDexModeSelect),
    .maxShowed = 9,
    .windowId = 0,
    .header_X = 0,
    .item_X = 10,
...
static const struct ListMenuItem sListMenuItems_NatDexsanityDexModeSelect[] = {
    {gText_PokemonList,                  LIST_HEADER},
    {gText_NumericalModeKanto,           DEX_MODE(NUMERICAL_KANTO)},
    {gText_NumericalModeNational,        DEX_MODE(NUMERICAL_NATIONAL)},
    {gText_NumericalModeDexsanity,       DEX_MODE(NUMERICAL_DEXSANITY)},
...
static const struct ListMenuTemplate sListMenuTemplate_NatDexsanityDexModeSelect = {
    .items = sListMenuItems_NatDexsanityDexModeSelect,
    .moveCursorFunc = MoveCursorFunc_DexModeSelect,
    .itemPrintFunc = ItemPrintFunc_DexModeSelect,
    .totalItems = NELEMS(sListMenuItems_NatDexsanityDexModeSelect),
    .maxShowed = 9,
    .windowId = 0,
    .header_X = 0,
    .item_X = 10,
...
static const struct WindowTemplate sWindowTemplate_DexInfo_EncounterArea = {
    .bg = 2,
    .tilemapLeft = 13,
    .tilemapTop = 2,
    .width = 16,
...
static const struct WindowTemplate sWindowTemplate_DexInfo_EncounterList = {
    .bg = 2,
    .tilemapLeft = 13,
    .tilemapTop = 4,
    .width = 16,
...
static const struct WindowTemplate sWindowTemplate_DexInfo_Evolutions = {
    .bg = 2,
    .tilemapLeft = 1,
    .tilemapTop = 7,
    .width = 12,
...
static const struct WindowTemplate sWindowTemplate_DexInfo_EvolutionList = {
    .bg = 2,
    .tilemapLeft = 1,
    .tilemapTop = 9,
    .width = 11,
...
static const u16 sPokedexRewards[NATIONAL_DEX_COUNT] = {0};
...
static void Task_PokedexScreen(u8 taskId)
{
    ...
    case 6:
        ...
        if (JOY_NEW(A_BUTTON))
        {
            ...
            case DEX_MODE(NUMERICAL_DEXSANITY):
                RemoveScrollIndicatorArrowPair(sPokedexScreenData->scrollArrowsTaskId);
...
static void DexScreen_InitGfxForTopMenu(void)
{
    ...
    if (IsNationalPokedexEnabled())
    {
        if (gArchipelagoOptions.isDexsanity)
            listMenuTemplate = sListMenuTemplate_NatDexsanityDexModeSelect;
        else
    ...
    {
        if (gArchipelagoOptions.isDexsanity)
            listMenuTemplate = sListMenuTemplate_KantoDexsanityDexModeSelect;
        else
...
static u16 DexScreen_CountMonsInOrderedList(u8 orderIdx)
{
    ...
    case DEX_ORDER_NUMERICAL_NATIONAL:
        ...
        break;
    case DEX_ORDER_NUMERICAL_DEXSANITY:
        for (i = 0, j = 0; i < NATIONAL_DEX_COUNT; i++)
        {
            ndex_num = i + 1;
            if (HasDexsanityItem(ndex_num))
            {
                seen = DexScreen_GetSetPokedexFlag(ndex_num, FLAG_GET_SEEN, FALSE);
                caught = DexScreen_GetSetPokedexFlag(ndex_num, FLAG_GET_CAUGHT, FALSE);
                if (seen)
                {
                    sPokedexScreenData->listItems[j].label = gSpeciesNames[NationalPokedexNumToSpecies(ndex_num)];
                    ret = j + 1;
                }
                else
                {
                ...
                sPokedexScreenData->listItems[j].index = (caught << 17) + (seen << 16) + NationalPokedexNumToSpecies(ndex_num);
                j++;
            }
        }
        break;
...
static void DexScreen_InitListMenuForOrderedList(const struct ListMenuTemplate * template, u8 order)
{
    ...
    case DEX_ORDER_NUMERICAL_NATIONAL:
        ...
        break;
    case DEX_ORDER_NUMERICAL_DEXSANITY:
        sPokedexScreenData->orderedListMenuTaskId = ListMenuInitInRect(template, sListMenuRects_OrderedList, sPokedexScreenData->dexsanityOrderMenuCursorPos, sPokedexScreenData->dexsanityOrderMenuItemsAbove);
        break;
    }
}
...
static void DexScreen_DestroyDexOrderListMenu(u8 order)
{
    ...
    case DEX_ORDER_NUMERICAL_NATIONAL:
        ...
        break;
    case DEX_ORDER_NUMERICAL_DEXSANITY:
        DestroyListMenuTask(sPokedexScreenData->orderedListMenuTaskId, &sPokedexScreenData->dexsanityOrderMenuCursorPos, &sPokedexScreenData->dexsanityOrderMenuItemsAbove);
        break;
    }
}
...
static void ItemPrintFunc_OrderedListMenu(u8 windowId, u32 itemId, u8 y, u16 index)
{
    ...
    if (seen)
    {
        DexScreen_PrintMonHMCompatibilityNumbers(sPokedexScreenData->numericalOrderWindowId, FONT_SMALL, species, 0xB8, y);
    }
}
...
static void Task_DexScreen_CategorySubmenu(u8 taskId)
{
    ...
    case 29:
        ...
        {
            if (sPokedexScreenData->encounterAreaInfo->totalPages > 1)
                DexScreen_DisplayNextAreaPage();
        }
        else if (JOY_NEW(DPAD_LEFT))
        {
            if (sPokedexScreenData->encounterAreaInfo->totalPages > 1)
                DexScDexScreen_DisplayPreviousAreaPage();
        }
        else if (JOY_NEW(DPAD_DOWN))
        {
            if (sPokedexScreenData->evolutionInfo->totalPages > 1)
                DexScreen_DisplayNextEvolutionPage();
        }
        else if (JOY_NEW(DPAD_UP))
        {
            if (sPokedexScreenData->evolutionInfo->totalPages > 1)
                DexScDexScreen_DisplayPreviousEvolutionPage();
        }
        break;
    case 30:
        DexScreen_DestroyInfoScreenResources();
        sPokedexScreenData->state = 21;
        break;
    case 31:
        DexScreen_DestroyInfoScreenResources();
...
static bool32 DexScreen_TryScrollMonsVertical(u8 direction)
{
    ...
    case DEX_ORDER_NUMERICAL_NATIONAL:
        ...
        break;
    case DEX_ORDER_NUMERICAL_DEXSANITY:
        cursorPos_p = &sPokedexScreenData->dexsanityOrderMenuCursorPos;
        itemsAbove_p = &sPokedexScreenData->dexsanityOrderMenuItemsAbove;
        break;
    }
...
static void DexScreen_PrintMonHMCompatibility(u8 windowId, u8 fontId, u16 species, u8 x, u8 y)
{
    DexScreen_AddTextPrinterParameterized(windowId, FONT_SMALL, gText_HM, x, y, 0);
    DexScreen_PrintMonHMCompatibilityNumbers(windowId, fontId, species, x + 40, y);
}
...
static void DexScreen_PrintMonHMCompatibilityNumbers(u8 windowId, u8 fontId, u16 species, u8 x, u8 y)
{
    u8 index;
    u8 buffer[32];
    u8 hm_characters[8] = {CHAR_1, CHAR_2, CHAR_3, CHAR_4, CHAR_5, CHAR_6, CHAR_7, CHAR_8};
    u8 i = 0;

    buffer[i++] = EXT_CTRL_CODE_BEGIN;
    buffer[i++] = EXT_CTRL_CODE_MIN_LETTER_SPACING;
    buffer[i++] = 5;
    for (index = 50; index < 58; index++)
    {
        if (CanSpeciesLearnTMHM(species, index))
            buffer[i++] = hm_characters[index - 50];
        else
            buffer[i++] = CHAR_SPACE;
    }
    buffer[i++] = EOS;

    DexScreen_AddTextPrinterParameterized(windowId, fontId, buffer, x, y, 0);
}
...
static u8 DexScreen_DrawMonDexPage(bool8 justRegistered)
{
    ...
    DexScreen_PrintMonWeight(sPokedexScreenData->windowIds[1], sPokedexScreenData->dexSpecies, 0, 38);
    DexScreen_PrintMonHMCompatibility(sPokedexScreenData->windowIds[1], FONT_SMALL, sPokedexScreenData->dexSpecies, 0, 48);
    DexScreen_DrawMonFootprint(sPokedexScreenData->windowIds[1], sPokedexScreenData->dexSpecies, 88, 38);
...
u8 DexScreen_DrawMonAreaPage(void)
{
    ...
    FillWindowPixelBuffer(sPokedexScreenData->windowIds[10], PIXEL_FILL(0));
    {
        ...
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[10], FONT_SMALL, gText_Area, (sWindowTemplate_AreaMap_Area.width * 8 - strWidth) / 4, 4, 0);
    }
...
u8 DexScreen_DrawMonInfoPage(void)
{
    ...
    sPokedexScreenData->windowIds[0] = AddWindow(&sWindowTemplate_DexInfo_EncounterArea);
    sPokedexScreenData->windowIds[1] = AddWindow(&sWindowTemplate_DexInfo_EncounterList);
    sPokedexScreenData->windowIds[2] = AddWindow(&sWindowTemplate_AreaMap_SpeciesName);
    sPokedexScreenData->windowIds[3] = AddWindow(&sWindowTemplate_AreaMap_MonIcon);
    sPokedexScreenData->windowIds[4] = AddWindow(&sWindowTemplate_AreaMap_MonTypes);
    sPokedexScreenData->windowIds[5] = AddWindow(&sWindowTemplate_DexInfo_Evolutions);
    sPokedexScreenData->windowIds[6] = AddWindow(&sWindowTemplate_DexInfo_EvolutionList);

    GetSpeciesPokedexAreaInfo(species, sPokedexScreenData->encounterAreaInfo);
    GetSpeciesEvolutionInfo(species, sPokedexScreenData->evolutionInfo);

    // Draw the mon icon
    FillWindowPixelBuffer(sPokedexScreenData->windowIds[3], PIXEL_FILL(0));
    ListMenu_LoadMonIconPalette(BG_PLTT_ID(10), species);
    ListMenu_DrawMonIconGraphics(sPokedexScreenData->windowIds[3], species, DexScreen_GetDefaultPersonality(species), 0, 0);
    PutWindowTilemap(sPokedexScreenData->windowIds[3]);
    CopyWindowToVram(sPokedexScreenData->windowIds[3], COPYWIN_GFX);

    // Print species name
    FillWindowPixelBuffer(sPokedexScreenData->windowIds[2], PIXEL_FILL(0));
    DexScreen_PrintMonDexNo(sPokedexScreenData->windowIds[2], FONT_SMALL, species, 0, 0);
    DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[2], FONT_NORMAL, gSpeciesNames[species], 3, 12, 0);
    PutWindowTilemap(sPokedexScreenData->windowIds[2]);
    CopyWindowToVram(sPokedexScreenData->windowIds[2], COPYWIN_GFX);
    ...
    if (monIsCaught)
    {
        BlitMenuInfoIcon(sPokedexScreenData->windowIds[4], 1 + gSpeciesInfo[species].types[0], 0, 1);
        if (gSpeciesInfo[species].types[0] != gSpeciesInfo[species].types[1])
            BlitMenuInfoIcon(sPokedexScreenData->windowIds[4], 1 + gSpeciesInfo[species].types[1], 32, 1);
    }
    PutWindowTilemap(sPokedexScreenData->windowIds[4]);
    CopyWindowToVram(sPokedexScreenData->windowIds[4], COPYWIN_GFX);

    // Print "Area Encounter"
    FillWindowPixelBuffer(sPokedexScreenData->windowIds[0], PIXEL_FILL(0));
    {
        s32 strWidth = GetStringWidth(FONT_SMALL, gText_EncounterArea, 0);
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[0], FONT_SMALL, gText_EncounterArea, (sWindowTemplate_DexInfo_EncounterArea.width * 8 - strWidth) / 2, 4, 0);
    }
    if (sPokedexScreenData->encounterAreaInfo->currentPage + 1 >= 10)
        ConvertIntToDecimalStringN(currentPage, sPokedexScreenData->encounterAreaInfo->currentPage + 1, STR_CONV_MODE_RIGHT_ALIGN, 2);
    else
        ConvertIntToDecimalStringN(currentPage, sPokedexScreenData->encounterAreaInfo->currentPage + 1, STR_CONV_MODE_RIGHT_ALIGN, 1);
    if (sPokedexScreenData->encounterAreaInfo->totalPages >= 10)
        ConvertIntToDecimalStringN(totalPages, sPokedexScreenData->encounterAreaInfo->totalPages, STR_CONV_MODE_RIGHT_ALIGN, 2);
    else
        ConvertIntToDecimalStringN(totalPages, sPokedexScreenData->encounterAreaInfo->totalPages, STR_CONV_MODE_RIGHT_ALIGN, 1);
    StringCopy(gStringVar5, currentPage);
    StringAppend(gStringVar5, gText_Slash);
    StringAppend(gStringVar5, totalPages);
    {
        s32 strWidth = GetStringWidth(FONT_SMALL, gStringVar5, 0);
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[0], FONT_SMALL, gStringVar5, sWindowTemplate_DexInfo_EncounterArea.width * 8 - strWidth - 2, 4, 0);
    }
    PutWindowTilemap(sPokedexScreenData->windowIds[0]);
    CopyWindowToVram(sPokedexScreenData->windowIds[0], COPYWIN_GFX);
    ...
    // Print Area List
    FillWindowPixelBuffer(sPokedexScreenData->windowIds[1], PIXEL_FILL(0));
    DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[1], FONT_SMALL, sPokedexScreenData->encounterAreaInfo->infoPages[sPokedexScreenData->encounterAreaInfo->currentPage], 6, 5, 0);
    PutWindowTilemap(sPokedexScreenData->windowIds[1]);
    CopyWindowToVram(sPokedexScreenData->windowIds[1], COPYWIN_GFX);
    ...
    {
        s32 strWidth = GetStringWidth(FONT_SMALL, gText_Evolutions, 0);
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[5], FONT_SMALL, gText_Evolutions, (sWindowTemplate_DexInfo_Evolutions.width * 8 - strWidth) / 2, 4, 0);
    }
    ConvertIntToDecimalStringN(currentPage, sPokedexScreenData->evolutionInfo->currentPage + 1, STR_CONV_MODE_RIGHT_ALIGN, 1);
    ConvertIntToDecimalStringN(totalPages, sPokedexScreenData->evolutionInfo->totalPages, STR_CONV_MODE_RIGHT_ALIGN, 1);
    StringCopy(gStringVar5, currentPage);
    StringAppend(gStringVar5, gText_Slash);
    StringAppend(gStringVar5, totalPages);
    {
        s32 strWidth = GetStringWidth(FONT_SMALL, gStringVar5, 0);
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[5], FONT_SMALL, gStringVar5, sWindowTemplate_DexInfo_Evolutions.width * 8 - strWidth - 2, 4, 0);
    }
    PutWindowTilemap(sPokedexScreenData->windowIds[5]);
    CopyWindowToVram(sPokedexScreenData->windowIds[5], COPYWIN_GFX);

    // Print Evolution List
    FillWindowPixelBuffer(sPokedexScreenData->windowIds[6], PIXEL_FILL(0));
    DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[6], FONT_SMALL, sPokedexScreenData->evolutionInfo->infoPages[sPokedexScreenData->evolutionInfo->currentPage], 6, 5, 0);
    PutWindowTilemap(sPokedexScreenData->windowIds[6]);
    CopyWindowToVram(sPokedexScreenData->windowIds[6], COPYWIN_GFX);

    // Draw the control info
    FillWindowPixelBuffer(1, PIXEL_FILL(15));
    DexScreen_AddTextPrinterParameterized(1, FONT_SMALL, gText_AreaEvolution, 8, 2, 4);
...
void DexScreen_DisplayNextAreaPage(void)
{
    if (sPokedexScreenData->encounterAreaInfo->currentPage == sPokedexScreenData->encounterAreaInfo->totalPages - 1)
        sPokedexScreenData->encounterAreaInfo->currentPage = 0;
    else
        sPokedexScreenData->encounterAreaInfo->currentPage++;
    DexScreen_DisplayAreaPage();
}

void DexScDexScreen_DisplayPreviousAreaPage(void)
{
    if (sPokedexScreenData->encounterAreaInfo->currentPage == 0)
       sPokedexScreenData->encounterAreaInfo->currentPage =  sPokedexScreenData->encounterAreaInfo->totalPages - 1;
    else
        sPokedexScreenData->encounterAreaInfo->currentPage--;
    DexScreen_DisplayAreaPage();
}
...
void DexScreen_DisplayNextEvolutionPage(void)
{
    if (sPokedexScreenData->evolutionInfo->currentPage == sPokedexScreenData->evolutionInfo->totalPages - 1)
        sPokedexScreenData->evolutionInfo->currentPage = 0;
    else
        sPokedexScreenData->evolutionInfo->currentPage++;
    DexScreen_DisplayEvolutionPage();
}

void DexScDexScreen_DisplayPreviousEvolutionPage(void)
{
    if (sPokedexScreenData->evolutionInfo->currentPage == 0)
       sPokedexScreenData->evolutionInfo->currentPage =  sPokedexScreenData->evolutionInfo->totalPages - 1;
    else
        sPokedexScreenData->evolutionInfo->currentPage--;
    DexScreen_DisplayEvolutionPage();
}
